...
void    SetUsbSpeed( UINT8 FullSpeed )  
{
    ...
    else
    {
        ...
        UH_SETUP |= bUH_PRE_PID_EN;                                            // 开启PRE PID			
    }
}
#endif
...
UINT8   CtrlGetHIDDeviceReport( UINT8 infc )  
{
    UINT8   s;
    UINT8   len;
    UINT8 tmp[]= {0x21,0x0a,0x00,0x00,0,0x00,0x00,0x00};
    tmp[4] = infc;
            ...
            if ( s == ERR_SUCCESS )
            {
                ...
                else if ( dv_cls == 0x00 && if_cls == USB_DEV_CLASS_PRINTER && ( (PXUSB_CFG_DESCR_LONG)TxBuffer ) -> itf_descr.bInterfaceSubClass == 0x01 )// 是打印机类设备
                {
                    ...
                    if((TxBuffer[19] == 5)&&(TxBuffer[20]&&0x80)){
                       ThisUsbDev.GpVar[0] = TxBuffer[20];                     //IN 端点											
                    }
                    else if((TxBuffer[19] == 5)&&((TxBuffer[20]&&0x80) == 0)){
                       ThisUsbDev.GpVar[1] = TxBuffer[20];                    //OUT 端点												
                    }		
                    if((TxBuffer[26] == 5)&&(TxBuffer[20]&&0x80)){
                       ThisUsbDev.GpVar[0] = TxBuffer[27];                     //IN 端点											
                    }
                    else if((TxBuffer[26] == 5)&&((TxBuffer[20]&&0x80) == 0)){
                       ThisUsbDev.GpVar[1] = TxBuffer[27];                    //OUT 端点												
                    }										
...
//                         }
                ...
                {
                    s = AnalyzeHidIntEndp( TxBuffer );                    // 从描述符中分析出HID中断端点的地址
                    ThisUsbDev.GpVar[1] = s & USB_ENDP_ADDR_MASK ;          // 保存中断端点的地址,位7用于同步标志位,清0
#if DE_PRINTF										
                    printf( "GetHubDescr: ");
#endif									
                    s = CtrlGetHubDescr( );
                    if ( s == ERR_SUCCESS )
                    {
                        ...
                        for( i = 0; i < TxBuffer[0]; i++ )
                        {
                            printf( "x%02X ",(UINT16)(TxBuffer[i]) );
												...
												printf("\n");
#endif												
                        ThisUsbDev.GpVar[0] = ( (PXUSB_HUB_DESCR)TxBuffer ) -> bNbrPorts;// 保存HUB的端口数量
                        if ( ThisUsbDev.GpVar[0] > HUB_MAX_PORTS )
                        {
                            ThisUsbDev.GpVar[0] = HUB_MAX_PORTS;// 因为定义结构DevOnHubPort时人为假定每个HUB不超过HUB_MAX_PORTS个端口
                        }
                        ...
                        if ( s == ERR_SUCCESS )
                        {
                            ...
                            for ( i = 1; i <= ThisUsbDev.GpVar[0]; i ++ )// 给HUB各端口都上电
                            {
                            ...
                            for ( i = 1; i <= ThisUsbDev.GpVar[0]; i ++ )         // 清除HUB各端口连接状态
                            {
                ...
                {			
                    ...
                    printf( "dv_cls %02x\n",(UINT16)dv_cls );
                    printf( "if_cls %02x\n",(UINT16)if_cls );
                    printf( "SUB %02x\n",(UINT16)( (PXUSB_CFG_DESCR_LONG)TxBuffer ) -> itf_descr.bInterfaceSubClass );									
#endif										
                    s = CtrlSetUsbConfig( cfg );                                     // 设置USB设备配置
                    if ( s == ERR_SUCCESS ) 
                    {
...
UINT8   EnumHubPort( ) 
{
    ...
    printf( "EnumHubPort\n" );
#endif	
    for ( i = 1; i <= ThisUsbDev.GpVar[0]; i ++ )                                       // 查询集线器的端口是否有变化
    {
