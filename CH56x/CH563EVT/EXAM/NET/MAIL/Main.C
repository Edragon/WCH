...
* Version            : V1.0
* Date               : 2013/11/15
* Description        : CH563 Mail DEMO
*                      (1)、CH563 Examples by KEIL;
*                      (2)、串口0输出监控信息,115200bps;
*                      (3)、ARM9-CH563单片机C语言的邮件收发演示程序
*******************************************************************************/
...
const UINT16 Pop3SourPrt = 7010;
const UINT8  Socketpop3IP[4] = {113,108,16,116};                                /* Socket pop3目的IP地址 */
        ...
        if( CheckType != uncheck ){
            ...
            if(i != CHECK_SUCCESS){
                if(sockeid == p_smtp->Socket) OrderType = SMTP_ERR_CHECK;
                   if(sockeid == p_pop3->Socket) OrderType = POP_ERR_CHECK;
#if  DEBUG
                printf("ERROR: %02x\n",(UINT16)i);
#endif
            }
...
#endif
        if(sockeid == p_pop3->Socket) p_pop3->DiscnntFlag = 1;
    }
...
void CH563NET_CreatTcpPop3( )
{
   ...
   TmpSocketInf.DesPort = 110;                                                  /* 设置目的端口 */
...
#ifdef send_mail 
    if(send_mail) CH563NET_CreatTcpSmtp( );
    CH563NET_SMTPInit( );                                                   
#endif
#ifdef    receive_mail
    if(receive_mail) CH563NET_CreatTcpPop3( );
    CH563NET_POP3Init( );
#endif
}
