...
void CH563NET_HandleSockInt(UINT8 sockeid,UINT8 initstat)
{
    ...
    if(initstat & SINT_STAT_CONNECT)                                            /* TCP连接中断 */
    {
        ...
        printf("socket status:%8x.%8x.%8x.%8x\n",(UINT32)SocketInf[0].ScokStatus,(UINT32)SocketInf[1].ScokStatus,(UINT32)SocketInf[2].ScokStatus,(UINT32)SocketInf[3].ScokStatus);             

#ifdef  KEEPLIVE_ENABLE
        CH563NET_SocketSetKeepLive( sockeid, 1 );                               /* 开启socket的KEEPLIVE功能（V06版本支持） */
#endif
