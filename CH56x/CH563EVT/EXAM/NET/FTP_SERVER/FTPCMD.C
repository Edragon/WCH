...
/*******************************************************************************
* Function Name  : CH563NET_FTPGetPortNum
* Description    : 获取端口号及IP地址
* Input          : recv_buff -接收到的数据
* Output         : None
* Return         : None
*******************************************************************************/
void CH563NET_FTPGetPortNum( char *recv_buff )
{
    UINT8 k,i;
...
/*******************************************************************************
* Function Name  : CH563NET_FTPGetFileName
* Description    : 获取名字
* Input          : recv_buff -接收到的数据
                   pBuf      -保存名字的缓冲区
* Output         : None
* Return         : None
*******************************************************************************/
void CH563NET_FTPGetFileName( char *recv_buff,char *pBuf )
{
     UINT8 k,i;
...
/*******************************************************************************
* Function Name  : CH563NET_FTPCmdRespond
* Description    : 根据client所发的命令，返回相应的回复
* Input          : recv_buff -接收到客户端发送的命令代码
* Output         : None
* Return         : None
*******************************************************************************/
void CH563NET_FTPCmdRespond( char *recv_buff ) 
{
    UINT8  s;
    UINT8  len;
    UINT16 port;
    ...
    if( strncmp("USER", recv_buff, 4) == 0 ){                                   /* 用户名 */
        ...
        CH563NET_FTPGetFileName( recv_buff,UserName );    
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[2],strlen(FTP_SERVICE_CMD[2]),ftp.SocketCtlConnect );
    }
    else if( strncmp("PASS", recv_buff, 4) == 0 ){                              /* 密码 */
#if Access_Authflag                                                             /* 如果需要检测用户名 */
        if(strncmp(pUserName, UserName, strlen(pUserName)) == 0){               /* 用户名正确 */
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[3],strlen(FTP_SERVICE_CMD[3]),ftp.SocketCtlConnect );
        }
        else{                                                                   /* 用户名错误 */
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[4],strlen(FTP_SERVICE_CMD[4]),ftp.SocketCtlConnect );
        }
#else
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[3],strlen(FTP_SERVICE_CMD[3]),ftp.SocketCtlConnect );
        ftp.AuthFlag = 1;
...
printf("have more authority\n");
        ...
        CH563NET_FTPGetPortNum( recv_buff );
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[5],strlen(FTP_SERVICE_CMD[5]),ftp.SocketCtlConnect );
        CH563NET_FTPClientDat( NumPort, CmdPortIP);          
    }
    else if( strncmp("LIST", recv_buff, 4) == 0 ){                              /* 获取当前列表信息 */
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[6],strlen(FTP_SERVICE_CMD[6]),ftp.SocketCtlConnect );
        CH563NET_FTPDataReady( "0" );
    }
        ...
        if(ftp.AuthFlag == 1){                                                  /* 如果有权限则支持上传 */
            ...
            CH563NET_FTPGetFileName( recv_buff,pFileName );
            memset((void *)SendBuf,'\0',sizeof(SendBuf));
            sprintf(SendBuf,"%s %s.\r\n",FTP_SERVICE_CMD[7],pFileName);
            CH563NET_FTPSendData( SendBuf,strlen(SendBuf),ftp.SocketCtlConnect );
        }
        else{                                                                   /* 无权限不支持的命令，关闭连接 */
            CH563NET_SocketClose( ftp.SocketDatMonitor,TCP_CLOSE_NORMAL );
            CH563NET_SocketClose( ftp.SocketCtlConnect,TCP_CLOSE_NORMAL );
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[7],strlen(FTP_SERVICE_CMD1[7]),ftp.SocketCtlConnect );
        }
    }
    else if( strncmp("RETR", recv_buff, 4) == 0){                               /* 客户端请求下载数据的命令（需要发送数据）*/
        if(recv_buff[5] == '/'){
            memset((void *)SendBuf,'\0',sizeof(SendBuf));    
            CH563NET_FTPGetFileName( recv_buff,SendBuf );
            len = strlen(ListName);
            sprintf(gFileName,&SendBuf[len]); 
        }
        else{
            memset((void *)gFileName,'\0',sizeof(gFileName));    
            CH563NET_FTPGetFileName( recv_buff,gFileName );
        } 
        memset((void *)SendBuf,'\0',sizeof(SendBuf));
        s = CH563NET_FTPFileOpen( gFileName,FTP_MACH_FILE );        
        if( s ){                                                                /* 文件存在，开始传输数据 */
            sprintf(SendBuf,"%s %s.\r\n",FTP_SERVICE_CMD[7],gFileName);
            CH563NET_FTPSendData( SendBuf,strlen(SendBuf),ftp.SocketCtlConnect );
            CH563NET_FTPDataReady( gFileName );
        }
        else{                                                                   /* 文件存在，发送错误响应 */
            sprintf(SendBuf,"%s（%s).\r\n",FTP_SERVICE_CMD[8],gFileName); 
            CH563NET_FTPSendData( SendBuf,strlen(SendBuf),ftp.SocketCtlConnect );
            s = CH563NET_SocketClose( ftp.SocketDatMonitor,TCP_CLOSE_NORMAL );
            mStopIfError(s); 
            ftp.SocketDatMonitor = 255;
        }
    }
    else if( strncmp("QUIT", recv_buff, 4) == 0 ){                              /* 退出登录 */
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD[9],strlen(FTP_SERVICE_CMD[9]),ftp.SocketCtlConnect );
    }
        ...
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[1],strlen(FTP_SERVICE_CMD1[1]),ftp.SocketCtlConnect );
    }
    else if( strncmp("PWD", recv_buff, 3) == 0 ){                               /* 打印目录 */
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[2],strlen(FTP_SERVICE_CMD1[2]),ftp.SocketCtlConnect );
    }
    else if( strncmp("CWD", recv_buff, 3) == 0 ){                               /* 打开目录 */
        memset((void *)ListName,'\0',sizeof(ListName));    
        CH563NET_FTPGetFileName( recv_buff,ListName );
        s = CH563NET_FTPFileOpen( ListName,FTP_MACH_LIST );
        if(s){
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[3],strlen(FTP_SERVICE_CMD1[3]),ftp.SocketCtlConnect ); 
        }
        else{
            memset((void *)SendBuf,'\0',sizeof(SendBuf));
            sprintf(SendBuf,"%s.\r\n",FTP_SERVICE_CMD[8]);
        ...
        memset((void *)SendBuf,'\0',sizeof(SendBuf));
        gLEN = CH563NET_FTPFileSize( gFileName );
        sprintf(SendBuf,"%s%d\r\n",FTP_SERVICE_CMD1[5],gLEN);
        CH563NET_FTPSendData( (char *)SendBuf,strlen(SendBuf),ftp.SocketCtlConnect );
    }
        ...
        if((ftp.AuthFlag == 1)&&(ftp.ListState == 0)){                          /* 如果有权限则支持上传 */
            ...
            CH563NET_FTPSendData( SendBuf,strlen(SendBuf),ftp.SocketCtlConnect );
            ftp.ListFlag = 3;
            CH563NET_FTPListRenew( ftp.ListFlag );
        }
        else{                                                                   /* 无权限不支持的命令，关闭连接 */
            CH563NET_SocketClose( ftp.SocketDatMonitor,TCP_CLOSE_NORMAL );
            CH563NET_SocketClose( ftp.SocketCtlConnect,TCP_CLOSE_NORMAL );
        ...
        if(ftp.AuthFlag == 1){                                                  /* 如果有权限则支持删除 */
            ...
            ftp.ListFlag = 4;
            CH563NET_FTPListRenew( ftp.ListFlag );
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[6],strlen(FTP_SERVICE_CMD1[6]),ftp.SocketCtlConnect );
        }
        else{
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[7],strlen(FTP_SERVICE_CMD1[7]),ftp.SocketCtlConnect );
        }
        ...
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[8],strlen(FTP_SERVICE_CMD1[8]),ftp.SocketCtlConnect );
    }
        ...
        if((ftp.AuthFlag == 1)&&(ftp.ListState == 0)){                          /* 如果有权限则支持删除 */
            ...
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD2[0],strlen(FTP_SERVICE_CMD2[0]),ftp.SocketCtlConnect );
        }
        else{
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[7],strlen(FTP_SERVICE_CMD1[7]),ftp.SocketCtlConnect );
        }
        ...
        if((ftp.AuthFlag == 1)&&(ftp.ListState == 0)){                          /* 如果有权限则支持删除 */
            ...
            ftp.ListFlag = 5;
            CH563NET_FTPListRenew( ftp.ListFlag );
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD2[1],strlen(FTP_SERVICE_CMD2[1]),ftp.SocketCtlConnect );
        }
        else{
            CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[7],strlen(FTP_SERVICE_CMD1[7]),ftp.SocketCtlConnect );
        }
        ...
        CH563NET_FTPSendData( (char *)FTP_SERVICE_CMD1[9],strlen(FTP_SERVICE_CMD1[9]),ftp.SocketCtlConnect );
    }
}
