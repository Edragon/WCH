...
*                      (3)、本程序用于演示TCP 服务端通讯，单片机收到数据后，回传给远端，ARM966,Thumb,小端
*******************************************************************************/
...
/* CH563相关定义 */
/* CH563相关定义 */
UINT8 MACAddr[6] = {0x02,0x03,0x04,0x05,0x16,0x17};                             /* CH563MAC地址 */
...
void CH563NET_HandleSockInt(UINT8 sockeid,UINT8 initstat)
{
    ...
    if(initstat & SINT_STAT_CONNECT)                                            /* TCP连接中断 */
    {
        ...
        CH563NET_ModifyRecvBuf(sockeid,(UINT32)SocketRecvBuf[sockeid],RECE_BUF_LEN);  
         i = CH563NET_SetSocketTTL( sockeid,10 );
#if CH563NET_DBG                                                                /* 产生此中断，CH563NET库内部会将此socket清除，置为关闭*/
        printf("CH563NET_SetSocketTTL:%d\n",(UINT16)i);
 #endif
...
/*******************************************************************************
* Function Name  : CH563NET_DHCPCallBack
* Description    : DHCP回
* Input          : None
* Output         : None
* Return         : 执行状态
*******************************************************************************/
UINT8 CH563NET_DHCPCallBack(UINT8 status,void *arg)
{
   UINT8 *p;
...
#endif 
    if(!i){
        CH563NET_DHCPStart(CH563NET_DHCPCallBack);                              /* 启动DHCP */
    }
