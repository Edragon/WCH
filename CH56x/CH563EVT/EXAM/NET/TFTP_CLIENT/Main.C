...
const UINT8 IPAddr[4] = {192,168,10,10};                                        /* CH563IP地址 */                                          
const UINT8 GWIPAddr[4] = {192,168,10,1};                                       /* CH563网关 */                                            
const UINT8 IPMask[4] = {255,255,255,0};                                        /* CH563子网掩码 */                                        
const UINT8 DESIP[4] = {192,168,10,100};                                        /* 目的IP地址 */                                           
UINT8 SocketId;                                                                 /* 保存socket索引，可以不用定义 */                         
UINT8 SocketRecvBuf[RECE_BUF_LEN];                                              /* socket接收缓冲区 */                                     
UINT8 MyBuf[RECE_BUF_LEN];                                                      /* 定义一个临时缓冲区 */                                   
UINT8 begin_flag=0;                                                                                                                        
UINT32 rcvnum=0;                                                                                                                           
UINT8 down_flag=0;                                                              /*文件下载完成标志*/                                       
UINT8 RCVBuffer[6*1024];                                                        /*保存从tftp服务器上下载的文件*/                           
...
void CH563NET_UdpServerRecv(struct _SCOK_INF *socinf,UINT32 ipaddr,UINT16 port,UINT8 *buf,UINT32 len)
{
    ...
    printf("port=%-8d len=%-8d",port,len);   
    sendport=port;
    Parse_Recv_Package(buf);  
    if(opcode==TFTP_DATA)                     
    {
      for(i=0;i<(len-4);i++){                                                   /*将接收到的数据保存在内部缓冲区中 */       
          RCVBuffer[rcvnum++] = buf[i+4];
      }
      if(len<516) {                        
         printf("\ndownload over ：len=%4d\n\n",rcvnum);  
         printf("%s\n",RCVBuffer);          
         rcvnum=0;
         down_flag=1;
        }                   
...
void CH563NET_CreatUpdSocket(void)
{
   ...
   TmpSocketInf.DesPort = 69;                                                   /* 设置目的端口 69*/
...
int main( void ) 
{
    ...
    while(1)
    {
       ...
       if(begin_flag){                                                          /*网络连接，发送下载请求*/        
          reqlen= Create_Request_Package(1,downfile);                           /*组装读请求包*/  
          len=reqlen;
          CH563NET_SocketUdpSendTo(0,ReqBuffer,&len,desip,69);      
          Delay_ms(200);
          if(len==reqlen){
             begin_flag=0;
       ...
       if(down_flag){                                                           /*下载完毕，上传文件*/
          reqlen= Create_Request_Package(2,upfile);                             /*组装写请求包*/  
          len=reqlen;
          CH563NET_SocketUdpSendTo(0,ReqBuffer,&len,desip,69);          
          Delay_ms(200);
          down_flag=0;
