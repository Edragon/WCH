...
//	UINT32 count;
	...
	flag = 0x00; 	
	if ( wTimeOutSec > DEF_USB_TIMEOUT / 1000 ) wTimeOutSec = DEF_USB_TIMEOUT / 1000;
	wTimeOutSec = wTimeOutSec * 50 + 5;  // 最少100uS
	for ( timeout = wTimeOutSec; timeout != 0; timeout -- ) { 	/* 等待传输结束 */
//		for ( count = (UINT32)( FREQ_SYS / DELAY_US_CLK_DIV * 20 ); count != 0; count -- );	/* 每间隔20uS左右查询一次 */ 
		Delay_us( 50 );
		if( spOldDumyqTD->bStatus_Active == 0 ) break;	// 完成,操作完成后自动从QH操作副本中复制回来
		if( EHCI_Regs->hc_usbsts & RB_PO_CHG_DET ) {  /* 查询设备是否断开 */
			flag = 0x02;
			EHCI_Regs->hc_usbsts = RB_PO_CHG_DET;
			break;
		}
	}
	gEHCILastStatus = spTempqHD->bOverlay_Status;      	// 保存上次清空累计来的执行状态,这是当前操作副本
//printf("gEHCILastStatus=%08lx\n",gEHCILastStatus);
//	gEHCILastStatus = 0X00;
	if ( spOldDumyqTD->bStatus_Active ) {  // 超时

		USBHOST_Asynchronous_Setting( USBHOST_OP_DISABLE );
		spOldDumyqTD->bStatus_Active = 0;    // 停止        
		spTempqHD->bOverlay_Status = 0x00;  // 停止操作副本
		spOldDumyqTD->bTerminate = 1;   // 断开链表
        spOldDumyqTD->bAlternateTerminate = 1;
		spOldDumyqTD->bErrorCounter = 0x03;                        
        Delay_us( 200 );  // 等待完成
		spOldDumyqTD->bStatus_Active = 0;    // 停止        
		spTempqHD->bOverlay_Status = 0x00;  // 停止操作副本
		flag |= 0x80;
		if( pRetLen ) *pRetLen = *pRetLen - spOldDumyqTD->bTotalBytes;	/* 保存长度 */
//		if( pRetLen ) *pRetLen = *pRetLen - spTempqHD->bOverlay_TotalBytes;	/* 保存长度 */
		spOldDumyqTD = spHeadqTD;  // 超时释放新TD
    }
    else {
    	spTempqHD->bOverlay_Status = 0x00;  // 清空以便再次累计错误
		if( pRetLen ) *pRetLen = *pRetLen - spOldDumyqTD->bTotalBytes;	/* 保存长度 */
    }
...
//    spReleaseqTD = ( qTD_Structure *)( ( (UINT32)( spTempqHD->bOverlay_NextqTD ) ) << 5 );  // 超时停在当前TD,完成则停在下一个TD即备用的新TD
	USBHOST_ReleaseStructure( EHCI_MEM_TYPE_QTD, ( UINT32 )spOldDumyqTD );	/* 回收qTD */
	if ( flag ) USBHOST_Asynchronous_Setting( USBHOST_OP_ENABLE );
	...
	{
		flag &= 0x7F;
		flag = timeout ? ( flag == 0x02 ? 0x02 : 0x01 ) : 0x04;  // 0x04=timeout,0x02=disconnect,0x01=error
	}
...
#endif   
#if 0
if ( flag || gEHCILastStatus ) {
	printf("HS=%x.%x\xd\xa", gEHCILastStatus,flag );//??
}
#endif
	/* 返回执行状态 */
	if( flag )  
	{
		if ( flag == 0x04 ) return( ERR_USB_TIMEOUT );
		else if ( flag == 0x02 ) return( USB_INT_DISCONNECT );
...
UINT8 USBHOST_Issue_Bulk( ATTACH_DEVICE_INFO *Device, qHD_Structure *pBlukqHD, UINT8 *pDatBuf, UINT16 *pSize, UINT8 Type )
{
	...
	/* 执行传输并等待返回结果 */
	return( USBHOST_Send_qTD( Device, spTempqTD, pBlukqHD, pSize, 200000 ) );	
}
...
UINT16 Android_Portocol;

/*******************************************************************************
* Function Name  : Android_Get_Protocol
* Description    : 安卓获取协议命令,01,00--表示为协议1.0,02,00--表示为协议2.0
* Input          : *Device---当前操作的设备
                   *buf------数据缓冲区
                   *len------数据长度                   
* Output         : None
* Return         : 返回当前命令执行状态
*******************************************************************************/
UINT8 Android_Get_Protocol(  ATTACH_DEVICE_INFO *Device, UINT8 *buf, UINT16 *len )
{
	UINT16 l;
	UINT8  status;
//0xc0, 0x33, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00	
	/* 填充SETUP命令包 */
	Ctl_Setup->bReqType = 0xc0;
	Ctl_Setup->bRequest = 0x33;
...
/*安卓命令码*/
#define		Manu_Name			0x00		//厂商信息
#define		Model_Name			0x01	    //模式名称
#define		Descr_Name			0x02		//描述
#define		Version				0x03		//版本
#define		URI					0x04		//URI
#define		Serial_Number		0x05	    //串行序列号
/*******************************************************************************
* Function Name  : Android_Set_Information
* Description    : 设置安卓设备信息命令
* Input          : *Device---当前操作的设备
                   *buf------数据缓冲区
                   *len------数据长度   
				   ID--------设置不同设备信息
				   00--------厂商信息“WCH\0”
				   01--------模式名称“CH563 Android Device\0”
				   02--------描述：“CH563 Accessory Device\0”
				   03--------版本：“1.0\0”
				   04--------URI:"http://wch.cn\0"
				   05--------Serial Number:WCH & CH563 & 1.0\0
* Output         : None
* Return         : 返回当前命令执行状态
*******************************************************************************/
UINT8 Android_Set_Information(  ATTACH_DEVICE_INFO *Device, UINT8 *buf, UINT16 *len ,UINT8 id)
{
	UINT16 l;
	UINT8  status;
//0xc0, 0x33, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00	
	/* 填充SETUP命令包 */
	Ctl_Setup->bReqType = 0x40;
	Ctl_Setup->bRequest = 0x34;
	Ctl_Setup->wValueL  = 0x00;
	Ctl_Setup->wValueH  = 0x00;
	Ctl_Setup->wIndexL  = id;
	Ctl_Setup->wIndexH  = 0x00;
	Ctl_Setup->wLengthL = *len;
	Ctl_Setup->wLengthH = 0x00;	
	/* 获取协议命令发送 */	
...
/*******************************************************************************
* Function Name  : Android_Start_AOA
* Description    : 开启AOA命令
* Input          : *Device---当前操作的设备                
* Output         : None
* Return         : 返回当前命令执行状态
*******************************************************************************/
UINT8 Android_Start_AOA(  ATTACH_DEVICE_INFO *Device, UINT8 *buf, UINT16 *len)
{
	UINT16 l;
	UINT8  status;
	/* 填充SETUP命令包 */
	Ctl_Setup->bReqType = 0x40;
	Ctl_Setup->bRequest = 0x35;
	Ctl_Setup->wValueL  = 0x00;
	Ctl_Setup->wValueH  = 0x00;
	Ctl_Setup->wIndexL  = 0x00;
	Ctl_Setup->wIndexH  = 0x00;
...
#if 1
	len = 2;
	status = Android_Get_Protocol( Device,buf,&len );							//如果未非安卓设备类则返回其他错误
#ifdef  MY_DEBUG_PRINTF 
	printf( "USBHOST_GetProtocol=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return NONE_ADNROID_DEVICE;				//非安卓设备类
#ifdef  MY_DEBUG_PRINTF 
	printf("buf[0]=%02x\n",buf[0]);
	printf("buf[1]=%02x\n",buf[1]);
#endif
   Android_Portocol	 = 	((UINT16)buf[0]<<8);
   Android_Portocol	 |= (buf[1]);
	
	strcpy( (char *)buf,(const char *)"WCH" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,Manu_Name );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Manu_Name=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_MANU_NAME_ERROR;			

	strcpy( (char *)buf,(const char *)"WCHUARTDemo" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,Model_Name );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Model_Name=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_MODEL_NAME_ERROR;				


	strcpy( (char *)buf,(const char *)"WCH Accessory Test" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,Descr_Name );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Descr_Name=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_DESCR_NAME_ERROR;				


	strcpy( (char *)buf,(const char *)"1.0" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,Version );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Version=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_VERSION_ERROR;				

	strcpy( (char *)buf,(const char *)"http://wch.cn" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,URI );
#ifdef  MY_DEBUG_PRINTF 
	printf( "URI=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_URI_ERROR;				


	strcpy( (char *)buf,(const char *)"WCHAccessory1" );
	len = strlen((const char *) buf )+1;
	status = Android_Set_Information( Device,buf,&len,Serial_Number );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Serial_Number=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return SET_SERAAL_NUMBER_ERROR;				

	len = 0;
	status = Android_Start_AOA( Device,buf,&len );
#ifdef  MY_DEBUG_PRINTF 
	printf( "Start AOA=%02x\n",status );
#endif
	if( status != USB_OPERATE_SUCCESS)return START_AOA_ERROR;						
	...
	/* (2)、复位USB设备*/
	status = USBHOST_EHCI_BusReset( Device );
//printf("aa=%02x\n",status);
...
* Input          : *Aoainfo----当前操作的U盘相关结构体
* Output         : None
* Return         : None
*******************************************************************************/
UINT8 MS_Bluk_QH_Allocate( AOA_INFO1 *Aoainfo )
{
	UINT32 temp;
//	UINT32 i;
#if 1
	/* 分配批量传输相关QH(qHD_Bulk_Out) */
	if( Aoainfo->BulkOutEp )
	{
		temp = USBHOST_GetStructure( EHCI_MEM_TYPE_QHD );
		if( temp == USB_MEM_ALLOCATE_ERROR )
		{
		...
		Aoainfo->qHD_Bulk_Out = ( qHD_Structure *)temp;						/* 分配qHD_Structure */
//printf("Aoainfo->qHD_Bulk_Out=%08lx\n",Aoainfo->qHD_Bulk_Out);	
	
		/* 初始化分配的OUT端点 */
		USBHOST_Allocate_QHD( &Aoainfo->Device, 
						      Aoainfo->qHD_Bulk_Out, 
						 	  EHCI_HD_TYPE_QH, 
							  EHCI_EP_TYPE_NO_CONTROL, 
							  0, 
							  Aoainfo->BulkOutEp, 
							  Aoainfo->BulkOutEpSize );
				
		/* 指针关联 */	
		USBHOST_qHD_Link_Insert( Aoainfo->qHD_Bulk_Out );						/* 将qHD_Bulk_Out链接到QH链表中 */
//printf("Aoainfo->qHD_Bulk_Out111=%08lx\n",Aoainfo->qHD_Bulk_Out);	
	}
#endif
#if 1
	...
	if( Aoainfo->BulkInEp )
	{
		...
		Aoainfo->qHD_Bulk_In = ( qHD_Structure *)temp;							/* 分配qHD_Structure */
//printf("Aoainfo->qHD_Bulk_In=%08lx\n",Aoainfo->qHD_Bulk_In);	
		/* 初始化分配的IN端点 */
		USBHOST_Allocate_QHD( &Aoainfo->Device, 
		                      Aoainfo->qHD_Bulk_In, 
		                      EHCI_HD_TYPE_QH, 
		                      ...
		                      0, 
		                      Aoainfo->BulkInEp, 
		                      Aoainfo->BulkInEpSize );	
	
		/* 指针关联 */	
		USBHOST_qHD_Link_Insert( Aoainfo->qHD_Bulk_In );						/* 将qHD_Bulk_In链接到QH链表中 */
//printf("Aoainfo->qHD_Bulk_In11111=%08lx\n",Aoainfo->qHD_Bulk_In);	
	}
#endif
...
*******************************************************************************/
UINT8 MS_CofDescrAnalyse( AOA_INFO1 *Aoainfo, UINT8 *pbuf )
{
	UINT16 i;
	UINT8  status;
...
//	}	
	Aoainfo->InterfNumber = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> itf_descr.bInterfaceNumber;  /* 保存接口号 */
	Aoainfo->BulkInEp  = 0;
	Aoainfo->BulkOutEp = 0;
	for( i = 0; i < 2; i ++ )   												/* 只分析前两个端点 */
	{
		...
		{  	
			...
			{	
				/* 批量IN端点 */
				Aoainfo->BulkInEp = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].bEndpointAddress & 0x0F;   /* IN端点 */
				Aoainfo->BulkInEpSize = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].wMaxPacketSizeH;
				Aoainfo->BulkInEpSize = Aoainfo->BulkInEpSize << 8;
				Aoainfo->BulkInEpSize += ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].wMaxPacketSizeL;				
			}
			else 
			{
				Aoainfo->BulkOutEp = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].bEndpointAddress & 0x0F;  /* OUT端点 */
				Aoainfo->BulkOutEpSize = ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].wMaxPacketSizeH;
				Aoainfo->BulkOutEpSize = Aoainfo->BulkOutEpSize << 8;
				Aoainfo->BulkOutEpSize += ( (PUSB_CFG_DESCR_LONG)pbuf ) -> endp_descr[ i ].wMaxPacketSizeL;				
			}
		}
	}
//	if( ( (PUSB_CFG_DESCR_LONG)pbuf ) -> itf_descr.bInterfaceClass != 0x08 || Aoainfo->BulkInEp == 0 || Aoainfo->BulkOutEp == 0 )
//	{  															 				/* 不是USB存储类设备,不支持 */
//		return( USB_OPERATE_ERROR ); 	  									    /* 终止操作,直接返回 */
//	}
//	Aoainfo->BulkInEp = 0X01;

#ifdef  MY_DEBUG_PRINTF 
	printf("Aoainfo->BulkInEp:%02x\n ",Aoainfo->BulkInEp);
	printf("Aoainfo->BulkOutEp:%02x\n ",Aoainfo->BulkOutEp);
	printf("Aoainfo->BulkInEpSize:%02x\n ",Aoainfo->BulkInEpSize);
	printf("Aoainfo->BulkOutEpSize:%02x\n ",Aoainfo->BulkOutEpSize);
#endif
	/*******************************分配大容量存储设备相关内存************************/
	status = MS_Bluk_QH_Allocate( Aoainfo );									/* USB主机批量传输相关QH、TD分配 */
	if ( status != USB_OPERATE_SUCCESS ) 
	{
...
/*发送数据*/
UINT8 AOA_OUT_Data( AOA_INFO1 *Aoainfo,UINT8 *pbuf,UINT16 len ){
	UINT8 status;
	status = USBHOST_Issue_Bulk( &Aoainfo->Device, Aoainfo->qHD_Bulk_Out, pbuf, &len, EHCI_QTD_PID_OUT );
	return status;
}


/*发送数据*/
UINT8 AOA_IN_Data( AOA_INFO1 *Aoainfo,UINT8 *pbuf,UINT16 len )
{
	UINT8 status;
	UINT32 temp;
	qTD_Structure *spTempqTD;
	...
	/* USB主机发起一个QTD */
	status = USBHOST_Issue_qTD( &Aoainfo->Device, spTempqTD, Aoainfo->qHD_Bulk_In );
	temp = 0;
	while(1)
	{
		status = USBHOST_Check_qTD( &Aoainfo->Device,spTempqTD, Aoainfo->qHD_Bulk_In, &len );
		if( status == USB_OPERATE_SUCCESS )
		{
			break;
		}
		temp++;
		Delay_ms( 1 );
		if( temp>=500 ){						//500ms的超时
			status = ERR_USB_TIMEOUT;
			break;
		}
	}

	USBHOST_Remove_qTD( &Aoainfo->Device, spTempqTD, Aoainfo->qHD_Bulk_In );
	return status;
}
