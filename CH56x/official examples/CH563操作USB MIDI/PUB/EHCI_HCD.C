...
UINT8  Setup_BackUp[ 8 ];														/* Setup包备份缓冲区 */

extern UINT8  bulk_sta;
/*******************************************************************************
...
{
	if( ( EHCI_Regs->hc_portsc & RB_PO_SUSP ) == 0 )  
	{
		return( USB_PARAMETER_ERROR );
	}
...
UINT8 USBHOST_EHCI_BusReset( ATTACH_DEVICE_INFO *Device )  
{
	...
	/* 恢复CH563内部地址为0 */
	Device->bAddress = Device-> bDeviceOnHub ? Device->bOnHubPortNumber + 0 + 1 : 0;	/* 保存当前操作的地址值 */
		...
		{	
			...
			if( total && pDatBuf )   											/* 需要进行数据收发 */
			{
				...
				{	
					...
					{
						memcpy( Setup_BackUp, pCmdPack, 8 );	  				/* 备份原Setup命令包 */
						USBHOST_EHCI_ClearEndpStall( Device, 0x00 );			/* 清除端点 */
						memcpy( back, Setup_BackUp, 8 );						/* 恢复原Setup命令包 */
						memcpy( Ctl_Setup, Setup_BackUp, 8 );					/* 恢复原Setup命令包 */
						continue;
					}
					tog = FALSE;  												/* 状态阶段为OUT */
				}		   
...
UINT8 USBHOST_Issue_Bulk( ATTACH_DEVICE_INFO *Device, qHD_Structure *pBlukqHD, UINT8 *pDatBuf, UINT16 *pSize, UINT8 Type )
{
	...
	/* 执行传输并等待返回结果 */
	return( USBHOST_Send_qTD( Device, spTempqTD, pBlukqHD, pSize, 1000 ) );	
}
...
UINT8 USBHOST_EHCI_Monitor( ATTACH_DEVICE_INFO *Device ) 
{
	UINT8  status;
	UINT8  buf[4] = {0,0,0,0};
	...
	Ctl_Setup->bRequest = 0x02;
	Ctl_Setup->wValueL  = 0x80;
	Ctl_Setup->wValueH  = 0x00;
	Ctl_Setup->wIndexL  = 0x00;
	Ctl_Setup->wIndexH  = 0x00;
	Ctl_Setup->wLengthL = 0x04;
	Ctl_Setup->wLengthH = 0x00;
	status = USBHOST_Issue_Control( Device, Ctl_Setup, buf, NULL  );			/* 执行控制传输 */

	Ctl_Setup->bReqType = 0x40;
	Ctl_Setup->bRequest = 0x02;
	Ctl_Setup->wValueL  = 0x20;
	Ctl_Setup->wValueH  = 0x00;
	Ctl_Setup->wIndexL  = 0x00;
	Ctl_Setup->wIndexH  = 0x00;
	Ctl_Setup->wLengthL = 0x04;
...
UINT8 USBHOST_EHCI_Enumerate( ATTACH_DEVICE_INFO *Device, UINT8 *pbuf )
{
	...
	/* (1)、以最大包获取设备描述符 */
#ifdef  MY_DEBUG_PRINTF 
	printf( "1.USBHOST_GetDeviceDescr:...\n" );
#endif
	...
	/* (4)、以实际长度获取设备描述符 */
#ifdef  MY_DEBUG_PRINTF 
	printf( "4. USBHOST_GetDeviceDescr:...\n" );
#endif
	len = 0x12;
	status = USBHOST_EHCI_GetDeviceDescr( Device, pbuf, &len  );  	 			/* 获取设备描述符 */
	if( status != USB_OPERATE_SUCCESS ) 
	...
	/* (6)、以实际长度获取配置描述符 */
#ifdef  MY_DEBUG_PRINTF
	printf( "6. USBHOST_GetConfigDescr:...\n " );
#endif
	...
	{	
#ifdef  MY_DEBUG_PRINTF
		printf( "Current Device is Not USB CLASS Device\n" );
#endif
		return( USB_OPERATE_ERROR ); 		 									/* 终止操作,直接返回 */
	}
