...
/* PLL config */
/* FREQ_OSC: 外部晶振   FREQ_VCO: PLL时钟   FREQ_SYS: 系统时钟 */
void SysFreq( void )
{
...
#endif
#if ( FREQ_VCO_MUL != 25 )
    R8_PLL_LOOP_DIV = FREQ_VCO_MUL ;	//倍频系数
#endif
#if ( FREQ_OSC_DIV != 3 )
    R8_PLL_PRE_DIV = FREQ_OSC_DIV ;		//分频系数
#endif	
#if ( FREQ_SYS_DIV != 4 || FREQ_VCO / 25000000 != 10 )	//输出时钟分频寄存器:RB_PLL_SYS_DIV+RB_PLL_ETH_DIV
    R8_PLL_OUT_DIV = 0x80 | ( FREQ_SYS_DIV << 4 ) & RB_PLL_SYS_DIV | ( FREQ_VCO / 25000000 ) & RB_PLL_ETH_DIV ;
#endif
#if ( MEM_DATA != 32 )
    R8_GLOB_MEM_CFG = 0x80 | ( MEM_DATA == 96 ? 0x03 : ( MEM_DATA == 64 ? 0x00 : 0x02 ) );	//全局配置寄存器,配置数据空间
#endif
