...
//idProduct字段。产品ID号，由于是第四个实验，我们这里取0x0004。
//注意小端模式，低字节应该在前。
 0x04,
 0x00,
...
code uint8 MouseReportDescriptor[]=
{
 ...
 //报告ID，这里定义鼠标报告的ID为1
 //为了减少程序修改量，这里依然保留报告ID。
 //事实上，该报告只有一个，所以可以不用报告ID。
 0x85, 0x01, //Report ID (1)
...
//字符串“《圈圈教你玩USB》之带鼠标的USB键盘”的Unicode编码
//8位小端格式
       ...
       switch((wValue>>8)&0xFF)
        {
         ...
         case REPORT_DESCRIPTOR:  //报告描述符
           ...
           Prints("报告描述符。\r\n");
          #endif
          //获取报告描述符时，wIndex中保存的是请求的接口号。
          //我们这里有两个接口，接口0用来实现键盘，接口1实现鼠标。
          switch(wIndex)
          {
           case 0:    //发送到接口0的请求
