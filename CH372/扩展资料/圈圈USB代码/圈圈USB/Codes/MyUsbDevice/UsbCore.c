...
//bDeviceClass字段。用户自定义USB设备，代码为0xFF
 0xFF,
...
code uint8 ConfigurationDescriptor[9+9+7+7+7+7]=
{
 ...
 //bNumEndpoints字段。非0端点的数目。本实例需要四个端点，因此该值为4。
 0x04,
 
 //bInterfaceClass字段。该接口所使用的类，未指定。该设备类在设备描述符中已指定。
 0x00,
...
void UsbEp1Out(void)
{
 ...
 //输出数据第一字节为LED状态，某位为1时，表示LED亮。
 LEDs=~Buf[0];
 
 //输出数据的第二字节非0时，清除发送计数器Count。
 if(Buf[1]!=0)
 {
  Count=0;
 }
