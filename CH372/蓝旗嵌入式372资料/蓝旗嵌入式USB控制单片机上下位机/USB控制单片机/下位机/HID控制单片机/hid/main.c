/***********************************************************************************
**************************************蓝旗嵌入式 *************************************
***********************CH372USB模块示例程序------HID控制单片机**************************
************http://lenchimcu.taobao.com QQ184930560 fengwei@jiangnan.edu.cn***************
***********************************************************************************/

#include "include.h"

/***********************************************************************************
****函数名称：以下为接收和发送数据缓冲区的定义
****函数作用：
****函数描述：
************************************************************************************/
...
void int0(void) interrupt 0 using 0
{
	...
	{
		case  USB_INT_EP2_OUT:  			//端点2下传成功 
			CH375_WRCMD(CMD_RD_USB_DATA);	//发出读数据命令
			length=CH375_REDAT();			//首先读出的是长度														
			for(i=0;i<length;i++)            //将数据放入缓冲区
				RDataBuf[i]=CH375_REDAT();
			break;
		
		case   USB_INT_EP2_IN:				//端点2上传成功,未处理
			CH375_WRCMD(CMD_UNLOCK_USB);	//释放缓冲区
			break;
		case   USB_INT_EP1_IN:	 			//端点1上传成功，未处理
			CH375_WRCMD(CMD_UNLOCK_USB);	//释放缓冲区
			break;
		case   USB_INT_EP1_OUT:	  			//端点1下传成功
			CH375_WRCMD(CMD_RD_USB_DATA);	//发出读数据命令
			if(length=CH375_REDAT())      //读取数据的长度
			{								
          ...
          //CH375_WR_CMD_PORT (CMD_UNLOCK_USB);	//释放缓冲区,如果前面没有CMD_RD_USB_DATA命令则使用本命令
			break;
		case   USB_INT_EP0_SETUP: 			//控制端点0建立成功
	    	CH375_WRCMD(CMD_RD_USB_DATA); //发送读数据指令
			length=CH375_REDAT();         //读取数据长度
			for(len=0;len!=length;len++)
			...
			if(length==0x08)
			{
				...
				if((c1=request.r.bmReuestType)&0x20)//类请求
				{   
