...
/********************************** (C) COPYRIGHT *******************************
* File Name          : USBHOST_KM.C
* Author             : WCH
* Version            : V1.2
* Date               : 2015/12/28
* Description        : 
 USB host example for CH559, start USB device under DP/DM port 
USB主机,有关USB-Host的应用例子,初始化和枚举当前Host端口连接的USB设备,有关Root-HUB/外部HUB的应用例子请参考USBH_HUB.C 
...
UINT8C	SetupSetBaud1152[] = { 0x40, 0xA1, 0x0c, 0x00, 0x03, 0xCC, 0x00, 0x00 };//115200bps
UINT8C	SetupSetBaud9600[] = { 0x40, 0xA1, 0x0c, 0x00, 0x02, 0x62, 0x00, 0x00 };//9600bps
UINT8C	SetupSetBaud600[] = { 0x40, 0xA1, 0x0c, 0x00, 0x01, 0x64, 0x00, 0x00 };//600bps
...
//#define	FREQ_SYS	20000000	// 系统主频20MHz
...
UINT8	EnableRootHubPort( void )  // 使能端口,相应的bUH_PORT_EN置1开启端口,设备断开可能导致返回失败
{
	...
	if ( USB_HUB_ST & bUHS_H0_ATTACH ) {  // 有设备
		if ( ( UHUB0_CTRL & bUH_PORT_EN ) == 0x00 ) {  // 尚未使能
			ThisUsbDev.DeviceSpeed = USB_HUB_ST & bUHS_DM_LEVEL ? 0 : 1;
			if ( ThisUsbDev.DeviceSpeed == 0 ) UHUB0_CTRL |= bUH_LOW_SPEED;  // 低速
		}
...
{
  UINT8 TxCnt,s,endp;
  endp = ThisUsbDev.GpVarOut;
	while ( len ) {
		mDelayuS( 200 );
		UH_TX_LEN = len >= 32 ? 32 : len;
		for ( TxCnt = 0; TxCnt != UH_TX_LEN; TxCnt ++ ) {
			TxBuffer[ TxCnt ] = *pBuf;
			pBuf ++;
		}
		s = USBHostTransact( USB_PID_OUT << 4 | endp & 0x7F, endp & 0x80 ? bUH_R_TOG | bUH_T_TOG : 0, 0 );  // OUT数据
		if ( s == ERR_SUCCESS ){ 
		len -= UH_TX_LEN;
		endp ^= 0x80;  // 同步标志翻转
		ThisUsbDev.GpVarOut = endp;  // 保存同步标志位
		}
    else return( s );
	}
  return ERR_SUCCESS;
...
/* 分析配置描述符,获取端点数据/各端点地址/各端点大小等,更新变量endp_addr和endp_size等 */
				...
				if ( if_cls == 0xff ) {  // 是CDC类
					// 从描述符中分析出端点的地址,因为固定是341，所以端点已知，不必分析
					ThisUsbDev.GpVar =  1;  // 保存中断端点的地址,位7用于同步标志位,清0
          ThisUsbDev.GpVarIn =  2;
          ThisUsbDev.GpVarOut =  2;
					if_cls = ( (PXUSB_CFG_DESCR_LONG)TxBuffer ) -> itf_descr.bInterfaceProtocol;
					s = CtrlSetUsbConfig( cfg );  // 设置USB设备配置
					if ( s == ERR_SUCCESS ) {
						ThisUsbDev.DeviceStatus = ROOT_DEV_SUCCESS;
							SetUsbSpeed( 1 );  // 默认为全速
              s = CtrlSetUsbCH341Buad();
