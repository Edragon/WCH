...
UINT8X  TxBuffer[ MAX_PACKET_SIZE*2 ] _at_ 0x0040 ;  // OUT, must even address
...
//          mDelayuS( 200 );                                     // 等待传输完成
            ...
            if ( RootHubDev[RootHubId].DeviceStatus == ROOT_DEV_DISCONNECT )
            {
                return( ERR_USB_DISCON );                        // USB设备断开事件
            }
            if ( RootHubDev[RootHubId].DeviceStatus == ROOT_DEV_CONNECTED )
            {
