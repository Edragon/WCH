...
//UINT8C DBFORMATCAP[]={0x00,0x00,0x00,0x08,(NUMOFBLOCK>>24)&0xFF,(NUMOFBLOCK>>16)&0xFF,(NUMOFBLOCK>>8)&0xFF,NUMOFBLOCK&0xFF,0x03,0x00,0x02,0x00}; //可格式化容量
UINT8C DBCAPACITY[]={(LASTLGCBLOCK>>24)&0xFF,(LASTLGCBLOCK>>16)&0xFF,(LASTLGCBLOCK>>8)&0xFF,LASTLGCBLOCK&0xFF,0x00,0x00,0x02,0x00};
UINT8C modesense3F[]={0x0b, 0x00, 0x00/*0x80*/, 0x08 , (NUMOFBLOCK>>24)&0xFF,(NUMOFBLOCK>>16)&0xFF,(NUMOFBLOCK>>8)&0xFF,NUMOFBLOCK&0xFF,0x00, 0x00, 0x02, 0x00 };   //写保护(0x80换成0x00可以去除写保护)
UINT8C DBR[512]={
...
void CH375bulkUpData(){											           //调用端点1上传数据
		...
		if(rootdir)       /* 改写根目录 */
		{
			memcpy(&Ep1Buffer[MAX_PACKET_SIZE],"WCH_CFG TXT ",12);
			Ep1Buffer[MAX_PACKET_SIZE+0x1A]=2;		
			Ep1Buffer[MAX_PACKET_SIZE+0x1C]= 64;
		}
		if(datfield)      /* 改写数据区 */
		{
			ReadDataFlash(0,64,&Ep1Buffer[MAX_PACKET_SIZE]);
...
void mCH375BulkDownData(){
	...
	if(dat_tran_flag)   //传输数据
	{
		if((UFI_Length.mDataLength&0x1ff)==0)
		{
			if((Ep1Buffer[0]=='f') && (memcmp(&Ep1Buffer[1],"usb",3)==0))
			{				
				FLASH_Write(0,&Ep1Buffer[0],64);   //同时写入Flash
			}
		}		
	}
