...
void parse_config_descr(unsigned char *p)
{
	...
	for(i=0;i<p[2];i+=l)
	{				
		if((Midi.AudioType == 0x03) && (p[i+1]==0x05))//端点描述符  bulkout_num
		{	
			if(p[i+3]==0x02)
			{
				if(p[i+2]&0x80)			//批量IN端点
				{
					Midi.BulkInEp = p[i+2]&0x0f;
					Midi.BulkInEpSize = p[i+4];
				}
				else                    //批量OUT端点
				{
					Midi.BulkOutEp = p[i+2]&0x0f;
					Midi.BulkOutEpSize = p[i+4];					
				}
			}
		}
		else if(p[i+1]==0x04)      //接口描述符
		{
			if((p[i+5]==0x01)&&(p[i+6]==0x03))          //MIDI Streaming
			{
				Midi.AudioType = 0x03;    
				Midi.IntfacNum = p[i+2];				
			}
		}
		l=p[i];
	}
